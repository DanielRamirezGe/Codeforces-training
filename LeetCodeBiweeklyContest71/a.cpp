...
vector <int> findPositions(vector <int> arr, int x) {
  ...
  while(!cola.empty()){
    ...
    for(int i=0; i<x && !cola.empty(); i++){
        pair<int,int> num = cola.front();
        cola.pop();
        if(num.first > max.first){
            max = num;
        }
        aux.push_back(num);
    }
    for(int i=0; i<aux.size(); i++){
      if(max.second != aux[i].second){
        cola.push({aux[i].first - (aux[i].first > 0? 1: 0), aux[i].second});
      }
      else{
